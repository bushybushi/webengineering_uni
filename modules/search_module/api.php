...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!empty($data['properties'])) {
            $propertyQuery = "INSERT INTO properties (declaration_id, type, location, area, value) VALUES (?, ?, ?, ?, ?)";
        ...
        if (!empty($data['vehicles'])) {
            $vehicleQuery = "INSERT INTO vehicles (declaration_id, type, model, year, value) VALUES (?, ?, ?, ?, ?)";
        ...
        if (!empty($data['liquid_assets'])) {
            $assetQuery = "INSERT INTO liquid_assets (declaration_id, type, amount) VALUES (?, ?, ?)";
        ...
        if (!empty($data['deposits'])) {
            $depositQuery = "INSERT INTO deposits (declaration_id, bank, account_number, amount) VALUES (?, ?, ?, ?)";
        ...
        if (!empty($data['insurance'])) {
            $insuranceQuery = "INSERT INTO insurance (declaration_id, type, company, amount) VALUES (?, ?, ?, ?)";
        ...
        // Insert debts
        if (!empty($data['debts'])) {
            $debtQuery = "INSERT INTO debts (declaration_id, type, creditor, amount) VALUES (?, ?, ?, ?)";
        ...
        if (!empty($data['business'])) {
            $businessQuery = "INSERT INTO bussiness (declaration_id, type, name, value) VALUES (?, ?, ?, ?)";
        ...
        if (!empty($data['differences'])) {
            $differencesQuery = "INSERT INTO differences (declaration_id, total_assets, total_liabilities, net_worth) VALUES (?, ?, ?, ?)";
            $differencesStmt = $pdo->prepare($differencesQuery);
            $differencesStmt->execute([
                $declarationId,
                $data['differences']['total_assets'],
                $data['differences']['total_liabilities'],
                $data['differences']['net_worth']
            ]);
        }

        // Insert previous incomes
        if (!empty($data['previous_incomes'])) {
            $previousIncomesQuery = "INSERT INTO previous_incomes (declaration_id, year, amount) VALUES (?, ?, ?)";
            $previousIncomesStmt = $pdo->prepare($previousIncomesQuery);
            $previousIncomesStmt->execute([
                $declarationId,
                $data['previous_incomes']['year'],
                $data['previous_incomes']['amount']
            ]);
        }
